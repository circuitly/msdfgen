...
static Napi::Value GenerateMTSDF(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();

    // Validate arguments: generateMTSDF(shape, options)
    if (info.Length() < 2) {
        Napi::TypeError::New(env, "Expected 2 arguments: (shape, options)")
            .ThrowAsJavaScriptException();
        return env.Null();
    ...
    // Assign edge colors (required before generateMTSDF)
    msdfgen::edgeColoringSimple(shape, 3.0, 0);

    // Allocate output bitmap (4 channels: R, G, B for MSDF + A for true SDF)
    msdfgen::Bitmap<float, 4> mtsdf(width, height);

    // Generate the MTSDF (Multi-channel True Signed Distance Field)
    // RGB channels carry the multi-channel SDF for sharp corner rendering,
    // alpha channel carries a true single-channel SDF for fallback at small sizes.
    msdfgen::Vector2 scaleVec(scale, scale);
    msdfgen::Vector2 translateVec(translateX, translateY);
    msdfgen::generateMTSDF(mtsdf, shape, range, scaleVec, translateVec);

    // Convert float [0,1] bitmap to uint8 [0,255] RGBA buffer
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            const float* pixel = mtsdf(x, y);
            size_t idx = (static_cast<size_t>(y) * static_cast<size_t>(width) + static_cast<size_t>(x)) * 4;
            data[idx + 0] = static_cast<uint8_t>(std::min(255.0f, std::max(0.0f, 256.0f * pixel[0])));
            data[idx + 1] = static_cast<uint8_t>(std::min(255.0f, std::max(0.0f, 256.0f * pixel[1])));
            data[idx + 2] = static_cast<uint8_t>(std::min(255.0f, std::max(0.0f, 256.0f * pixel[2])));
            data[idx + 3] = static_cast<uint8_t>(std::min(255.0f, std::max(0.0f, 256.0f * pixel[3])));
        }
    }
...
static Napi::Object Init(Napi::Env env, Napi::Object exports) {
    ...
    exports.Set("generateMTSDF", Napi::Function::New(env, GenerateMTSDF));
    return exports;
}
